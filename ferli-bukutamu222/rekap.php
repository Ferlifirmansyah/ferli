...
<table class="table class text-center bg-info-emphasis mt-3">
  <thead class="table-secondary">
    ...
    $tampil=$koneksi->query("select * from bukutamu_ferli");
  ...
  $bukutamu_ferli=$_GET['cari'];
 
    $tampil=$koneksi->query("select * from bukutamu_ferli where tanggal between'$_GET[awal]' and '$_GET[akhir]'");
    while ($a=$tampil ->fetch_array()){
       @$no++;
       ?>
  <tbody class="table">
